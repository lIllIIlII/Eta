...
安全设置另有运行期限制：**AI 操控手机需手动允许**（打开应用、点击、输入、改系统设置等手机操作逐次确认，输入栏的免确认开关不再生效）和**禁止 AI 访问指定路径**（调试用：列出的绝对路径及其子路径拒绝读取、写入与出现在命令中，同时写入系统提示）。
...
设置页的「检查更新」通过 GitHub Release 判断新版本：先访问 GitHub API，失败时改用发布页解析标签与安装包名，仍失败会提示打开发布页。下载安装包时依次尝试官方地址与 `gh-proxy`、`ghproxy`、`ghfast` 等国内加速前缀（也可开启「优先使用国内 GitHub 加速镜像」让镜像先行），每个源都校验包名与 versionCode，全部失败才会报错，下载过程随时可以取消。Eta 不再包含远程热更新配置，行为只由已安装的 APK 决定。
